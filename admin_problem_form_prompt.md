# AdminProblemForm — To'liq Qayta Yozish
## Barcha kritik muammolar hal qilingan holda
...
```bash
cd frontend
npm install @uiw/react-md-editor jszip lucide-react
...
## 1. BACKEND — MODEL O'ZGARTIRISH

### `apps/problems/models.py`

TestCase modeliga `file_number` field qo'sh:
...
class TestCase(models.Model):
    ...
    # KRITIK FIX: fayl nomi saqlanadi — DB id bilan fayl raqami
    # o'rtasidagi desync muammosi hal bo'ladi
...
### `apps/problems/views.py` ga quyidagilarni qo'sh:
...
# ─────────────────────────────────────────────────────────
# ZIP IMPORT — KRITIK FIX: server-side parse
# (browser da emas, backend da — katta fayllar uchun)
# ─────────────────────────────────────────────────────────
...
from apps.problems.views import (
    ProblemTestCasesView,
    TestCaseUpdateView,
    BulkTestCaseImportView,
    ZipTestCaseImportView,
)

urlpatterns += [
    path('api/admin/problems/<slug:slug>/testcases/',
        ProblemTestCasesView.as_view()),
    path('api/admin/problems/<slug:slug>/testcases/bulk/',
        BulkTestCaseImportView.as_view()),
    path('api/admin/problems/<slug:slug>/testcases/zip/',
        ZipTestCaseImportView.as_view()),
    path('api/admin/testcases/<int:pk>/',
...
### `src/api/admin.js` ga qo'sh:

```javascript
// Test cases
getTestCases:    (slug) =>
    api.get(`/api/admin/problems/${slug}/testcases/`),

addTestCase:     (slug, data) =>
    api.post(`/api/admin/problems/${slug}/testcases/`, data),

updateTestCase:  (id, data) =>
    api.patch(`/api/admin/testcases/${id}/`, data),

deleteTestCase:  (id) =>
    api.delete(`/api/admin/testcases/${id}/`),

bulkImport:      (slug, data) =>
    api.post(`/api/admin/problems/${slug}/testcases/bulk/`, data),

// KRITIK FIX: ZIP server-side parse
zipImport:       (slug, formData) =>
    api.post(`/api/admin/problems/${slug}/testcases/zip/`, formData, {
...
## 5. FRONTEND — AdminProblemForm.jsx TO'LIQ QAYTA YOZ

```
Fayl: src/admin/pages/AdminProblemForm.jsx
```

### IMPORTLAR (barcha kerakli):
...
### YORDAMCHI KOMPONENTLAR (bir faylning oxiriga qo'sh):
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                >
                    {test.saveState === 'loading' ? <><Spinner/> Saqlanmoqda...</>
                     : test.saveState === 'success' ? <><Check size={11}/> Saqlandi</>
                     : test.saveState === 'error'   ? <><AlertCircle size={11}/> Xato</>
                     : <><Save size={11}/> Saqlash</>}
                </button>
            </div>
        </div>
    )
...
```
1. Xato yo'qoldi:
   ✅ "Zap is not defined" — lucide-react dan import qilindi
   ✅ npm run dev xatosiz ishlaydi

2. Tab 1 — Masala:
   ✅ Slug preview title yozilganda yangilanadi
   ✅ Markdown preview almashtirish ishlaydi
   ✅ Difficulty kartalar ishlaydi
   ✅ Time/Memory chips ishlaydi
   ✅ Teglar qo'shish ishlaydi
   ✅ Saqlash: loading → success → idle

3. Tab 2 — Testlar (KRITIK FIXLAR):
   ✅ "+ Test qo'shish" ishlaydi
   ✅ Saqlash → file_number saqlanadi (DB id emas!)
   ✅ Tahrirlash → file_number bo'yicha fayl yangilanadi
   ✅ O'chirish → fayl ham o'chiriladi
   ✅ Race condition yo'q (save queue)
   ✅ Bulk text import → parse + server ga yuborish
   ✅ ZIP import → server-side parse (katta fayllar OK)

4. Backend:
   ✅ testcases/{slug}/ papkasi yaratiladi
   ✅ {n}.in va {n}.out fayllar yoziladi
   ✅ file_number DB da saqlanadi
   ✅ ZIP → 50MB gacha qabul qiladi
```
